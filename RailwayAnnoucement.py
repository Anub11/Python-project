...
def generateAnnoucement(filename):
    ...
    for index, item in df.iterrows():
        textToSpech(item['from'], '2_hindi.mp3')

        textToSpech(item['via'], '4_hindi.mp3')

        textToSpech(item['to'], '6_hindi.mp3')

        textToSpech(item['train_no']+" "+item['train_name'], '8_hindi.mp3')

        textToSpech(item['platform'], '10_hindi.mp3')

        audios = [f"{i}_hindi.mp3" for i in range(1,12)]

        annoucement = margeAudios(audios)
